...
void draw_detections_cv_v3(mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output)
{
    ...
    std::string namanya = "";
    std::string namanya3 = "";
    std::string kumpul_nama = "";
    char kumpul_nama_array[30][4096] ={0};
    int urut1 = 0,urut2=0,urut3=0,urut4=0;

    //            membuat batas di tengah atau garis bantu
                vector< Point> contour;
                contour.push_back(Point(250, 180));
                contour.push_back(Point(500, 180));
                contour.push_back(Point(550, 410));
                contour.push_back(Point(210, 410));
                contour.push_back(Point(250, 180));
    //            contour.push_back(Point(100, 100));
                ...
                // draw the polygon
                cv::polylines(*show_img, &pts, &npts, 1, false, Scalar(0, 255, 0));
                cv::rectangle(*show_img, Point(275,200),Point(550,400),Scalar(229,225,0),2,8,0);
    //            batas pembuatan garis bantu////////////////////////////////
    for (i = 0; i < num; ++i) {
//        num adalah banyaknya objek yang terdeteksi
        //printf("%d ini num    ",num);
        char labelstr[4096] = { 0 };
        int class_id = -1;
//        int batas = 0;
        int nomor =0;
        for (j = 0; j < classes; ++j) {
            //'classes adalah banyaknya object yang ada di coco dataset sebanyak 80
            int show = strncmp(names[j], "dont_show", 9);
//            batas++;
//            bool L = (thresh <dets[i].prob[j]);
//            bool M = (show);
//            if(batas == 1){
//            printf("%d ini ***************** %d",M,L);
//            }
//            if(batas !=1){
//                printf("##%d banding %d",M,L);
//            }
            if (dets[i].prob[j] > thresh && show) {
                // yang berpengaruh bool hasil dari dets[i].prob[j] > thresh karena bool show selalu 1
                if (class_id < 0) {
                    strcat(labelstr, names[j]);

                    std::string namanya2(labelstr);
                    namanya = namanya2;
//                    mendeteksi label dan menyimpannya di array
                    kumpul_nama =  (labelstr);
                    if (strcmp(labelstr,kumpul_nama_array[0]) == 0){
                        strcpy(kumpul_nama_array[0],kumpul_nama.c_str());
                        kumpul_banyak[0]++;
                    }
                    else if (strcmp(labelstr,kumpul_nama_array[0])!= 0 && strcmp(kumpul_nama_array[0],"")== 0){
                        strcpy(kumpul_nama_array[0],kumpul_nama.c_str());
                        kumpul_banyak[0]++;
                        urut1 = 1;
                    }
                    else if (strcmp(labelstr,kumpul_nama_array[1]) == 0 ){
                        strcpy(kumpul_nama_array[1],kumpul_nama.c_str());
                        kumpul_banyak[1]++;
                    }
                    else if (strcmp(labelstr,kumpul_nama_array[1])!= 0 && strcmp(kumpul_nama_array[1],"")== 0 && urut1 == 1){
                        strcpy(kumpul_nama_array[1],kumpul_nama.c_str());
                        kumpul_banyak[1]++;
                        urut2 = 1;
                    }
                    else if (strcmp(labelstr,kumpul_nama_array[3]) == 0){
                        strcpy(kumpul_nama_array[3],kumpul_nama.c_str());
                        kumpul_banyak[3]++;
                    }
                    else if (strcmp(labelstr,kumpul_nama_array[3])!= 0 && strcmp(kumpul_nama_array[2],"")== 0 && urut2 == 1){
                        strcpy(kumpul_nama_array[3],kumpul_nama.c_str());
                        kumpul_banyak[3]++;
                        urut3 = 1;
                    }
                    else if (strcmp(labelstr,kumpul_nama_array[4]) == 0){
                        strcpy(kumpul_nama_array[4],kumpul_nama.c_str());
                        kumpul_banyak[4]++;
                    }
                    else if (strcmp(labelstr,kumpul_nama_array[4])!= 0 && strcmp(kumpul_nama_array[3],"")== 0 && urut3 == 1){
                        strcpy(kumpul_nama_array[4],kumpul_nama.c_str());
                        kumpul_banyak[4]++;
                        urut4 = 1;
                    }
                    else if (strcmp(labelstr,kumpul_nama_array[5]) == 0){
                        strcpy(kumpul_nama_array[5],kumpul_nama.c_str());
                        kumpul_banyak[5]++;
                    }
                    else if (strcmp(labelstr,kumpul_nama_array[5])!= 0 && strcmp(kumpul_nama_array[4],"")== 0 && urut4 == 1){
                        strcpy(kumpul_nama_array[5],kumpul_nama.c_str());
                        kumpul_banyak[5]++;

                    }
// debug ///////////////////////////////////////////////////////
//                    while(strcmp(labelstr,kumpul_nama_array[nomor]) != 0 ){
//                        cek array sama dengan data
//                        if(strcmp(labelstr,kumpul_nama_array[nomor]) == 0 ){
                            //mengubah ke string
//                            kumpul_nama = (labelstr);
                            //memasukan ke array char sekaligus di mengubah dari string kembali ke array char
//                            strcpy(kumpul_nama_array[nomor],kumpul_nama.c_str());
//                            printf("%s + ",kumpul_nama_array[nomor+1]);
//                            kumpul_banyak[nomor]++;
//                            break;

//                        }
//                        cek array kosong
//                        else if(strcmp(kumpul_nama_array[nomor],"") == 0 ){
                            //mengubah ke string
//                            kumpul_nama = (labelstr);
                            //memasukan ke array char sekaligus di mengubah dari string kembali ke array char
//                            strcpy(kumpul_nama_array[nomor],kumpul_nama.c_str());
...
//                    menampilkan jumlah pendeteksian sesuai objek
                    if(strcmp(labelstr,kumpul_nama_array[0])==0){
                        printf("%s: %.0f%% %d", names[j], dets[i].prob[j] * 100, kumpul_banyak[0]);
                    }
                    if(strcmp(labelstr,kumpul_nama_array[1])==0){
                        printf("%s: %.0f%% %d", names[j], dets[i].prob[j] * 100, kumpul_banyak[1]);
                    }
                    if(strcmp(labelstr,kumpul_nama_array[2])==0){
                        printf("%s: %.0f%% %d", names[j], dets[i].prob[j] * 100, kumpul_banyak[2]);
                    }
                    if(strcmp(labelstr,kumpul_nama_array[3])==0){
                        printf("%s: %.0f%% %d", names[j], dets[i].prob[j] * 100, kumpul_banyak[3]);
                    }
                    if(strcmp(labelstr,kumpul_nama_array[4])==0){
                        printf("%s: %.0f%% %d", names[j], dets[i].prob[j] * 100, kumpul_banyak[4]);
                    }
// batas debug ///////////////////
                    class_id = j;
                    char buff[10];
                    ...
                    //editan
                    //merubah menjadi string dari array char untuk digunakan di tampilan GUI berhasil
                    std::string namanya1(labelstr);
                    namanya3 = namanya1;

                    //Merubah kembali dari string to array char berhasil
//                    char coba[30] = {0};
//                    strcpy(coba ,  namanya.c_str());
//                    printf("$%s",coba);
                    //printf("%s", namanya);
                }
                else {
...
//            batas pembuatan garis bantu////////////////////////////////
            char nama_gui[30] = {0};
            strcpy(nama_gui ,  namanya.c_str());
            std::string nama_gui_tampil = "";

            if(strcmp(nama_gui,kumpul_nama_array[0])==0){
//                printf("%s: %.0f%% %d", names[j], dets[i].prob[j] * 100, kumpul_banyak[0]);
                nama_gui_tampil = std::to_string(kumpul_banyak[0]);
//                cv::putText(*show_img, namanya + labelstr  , pt_text, cv::FONT_HERSHEY_COMPLEX_SMALL, font_size, black_color, 2*font_size, CV_AA);
                cv::putText(*show_img, namanya + nama_gui_tampil + labelstr , pt_text, cv::FONT_HERSHEY_COMPLEX_SMALL, font_size, black_color, 2*font_size, CV_AA);
            }
            if(strcmp(nama_gui,kumpul_nama_array[1])==0){
//                printf("%s: %.0f%% %d", names[j], dets[i].prob[j] * 100, kumpul_banyak[1]);
                nama_gui_tampil = std::to_string(kumpul_banyak[1]);
//                cv::putText(*show_img, namanya + labelstr  , pt_text, cv::FONT_HERSHEY_COMPLEX_SMALL, font_size, black_color, 2*font_size, CV_AA);
                cv::putText(*show_img, namanya  + nama_gui_tampil + labelstr , pt_text, cv::FONT_HERSHEY_COMPLEX_SMALL, font_size, black_color, 2*font_size, CV_AA);
            }
            if(strcmp(nama_gui,kumpul_nama_array[2])==0){
//                printf("%s: %.0f%% %d", names[j], dets[i].prob[j] * 100, kumpul_banyak[2]);
                nama_gui_tampil = std::to_string(kumpul_banyak[2]);
//                cv::putText(*show_img, namanya + labelstr  , pt_text, cv::FONT_HERSHEY_COMPLEX_SMALL, font_size, black_color, 2*font_size, CV_AA);
                cv::putText(*show_img, namanya + nama_gui_tampil + labelstr , pt_text, cv::FONT_HERSHEY_COMPLEX_SMALL, font_size, black_color, 2*font_size, CV_AA);
            }
            if(strcmp(nama_gui,kumpul_nama_array[3])==0){
//                printf("%s: %.0f%% %d", names[j], dets[i].prob[j] * 100, kumpul_banyak[3]);
                nama_gui_tampil = std::to_string(kumpul_banyak[3]);
//                cv::putText(*show_img, namanya + labelstr  , pt_text, cv::FONT_HERSHEY_COMPLEX_SMALL, font_size, black_color, 2*font_size, CV_AA);
                cv::putText(*show_img, namanya + nama_gui_tampil + labelstr , pt_text, cv::FONT_HERSHEY_COMPLEX_SMALL, font_size, black_color, 2*font_size, CV_AA);
            }
            if(strcmp(nama_gui,kumpul_nama_array[4])==0){
//                printf("%s: %.0f%% %d", names[j], dets[i].prob[j] * 100, kumpul_banyak[4]);
                nama_gui_tampil = std::to_string(kumpul_banyak[4]);
//                cv::putText(*show_img, namanya + labelstr  , pt_text, cv::FONT_HERSHEY_COMPLEX_SMALL, font_size, black_color, 2*font_size, CV_AA);
                cv::putText(*show_img, namanya + nama_gui_tampil + labelstr , pt_text, cv::FONT_HERSHEY_COMPLEX_SMALL, font_size, black_color, 2*font_size, CV_AA);
            }
//            cv::putText(*show_img, namanya + labelstr  , pt_text, cv::FONT_HERSHEY_COMPLEX_SMALL, font_size, black_color, 2*font_size, CV_AA);
